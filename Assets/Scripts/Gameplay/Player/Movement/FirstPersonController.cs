...
public class FirstPersonController : MonoBehaviour
{
    ...
    public struct ControllerState
    {
        ...
        {
            get => (StateFlags & (uint)StateFlag.IsSliding) != 0;
            set => SetFlag(StateFlag.IsSliding, value);
        }

        [GhostField(SendData = false)]
        public bool IsSprinting
        {
            get => (StateFlags & (uint)StateFlag.IsSprinting) != 0;
            set => SetFlag(StateFlag.IsSprinting, value);
        }
    ...
    {
        float targetHeight = state.IsSliding ? 0.90f : (state.IsCrouching ? 1.20f : 1.80f);
        if (m_Controller != null && m_Controller.enabled)
        {
            m_Controller.height = Mathf.MoveTowards(m_Controller.height, targetHeight, 10.0f * deltaTime);
        ...
        in PlayerInput input, in ControllerConsts consts, float deltaTime)
    {
        ...
        {
            bool wantsSprint = input.Sprint && input.MoveInput.y > 0.1f;
            bool wantsCrouch = input.Crouch;

            if (state.IsSliding)
            {
                const float k_SlideDuration = 1.0f;
                // If player released crouch during the slide or jumped, end slide immediately
                if (!wantsCrouch || state.MovementType != MovementType.Standing)
                {
                    state.IsSliding = false;
                    state.IsCrouching = wantsCrouch;
                    state.IsSprinting = wantsSprint && !wantsCrouch;
                    stateConsts = wantsCrouch ? consts.Crouch : (wantsSprint ? consts.Sprint : consts.Walk);
                }
                else if (state.TimeInState < k_SlideDuration)
                {
                    // Decaying slide speed from Slide speed (9.0 m/s) down to Crouch speed (3.2 m/s)
                    float progress = math.clamp(state.TimeInState / k_SlideDuration, 0f, 1f);
                    stateConsts = consts.Slide;
                    stateConsts.Speed = math.lerp(consts.Slide.Speed, consts.Crouch.Speed, progress);
                    state.IsCrouching = true;
                    state.IsSprinting = false;
                }
                else
                {
                    // Slide finished naturally -> transition to standard Crouch (prevents infinite slide loop while holding C)
                    state.IsSliding = false;
                    state.IsCrouching = true;
                    state.IsSprinting = false;
            ...
            {
                // Trigger slide ONLY if actively sprinting forward and pressing crouch
                if (wantsCrouch && wantsSprint && state.IsSprinting && state.MovementType == MovementType.Standing)
                {
                    state.IsSliding = true;
                    state.TimeInState = 0f;
                    stateConsts = consts.Slide;
                    stateConsts.Speed = consts.Slide.Speed;
                    state.IsCrouching = true;
                    state.IsSprinting = false;
                }
                else if (wantsCrouch)
                ...
                else
                {
                    ...
                    stateConsts = wantsSprint ? consts.Sprint : consts.Walk;
                }
            }
        }
    }
