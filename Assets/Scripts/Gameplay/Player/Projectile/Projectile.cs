            ...
            if (weaponData.Behavior == ProjectileBehavior.AreaOfEffect)
            {
                ...
                foreach (var playerCollider in playersInRadius)
                {
                        ...
                        playerGhostLookup.HasComponent(hitGhostObject.LinkedEntity))
                    {
                        ...
                        bool isFatal = (healthBeforeDamage > 0 && targetPredictedPlayer.ValueRO.CurrentHealth <= 0);
                        HitmarkerManager.TriggerHit(isFatal ? HitType.Kill : HitType.Body);
            ...
            else // DirectDamage
            {
                    ...
                    playerGhostLookup.HasComponent(hitGhostObject.LinkedEntity))
                {
                    ...
                    if (targetNetworkId != shooterNetworkId)
                    {
                        ...
                        // Headshot detection (impact point near player head height)
                        bool isHeadshot = (impactPosition.y - hitObject.transform.position.y) > 1.35f;
                        float appliedDamage = isHeadshot ? (weaponData.Damage * 1.8f) : weaponData.Damage;

                        targetPredictedPlayer.ValueRW.CurrentHealth -= appliedDamage;
                        targetPredictedPlayer.ValueRW.ControllerState.IsHit = true;
                        targetPredictedPlayer.ValueRW.LastDamageAmount = appliedDamage;
                        targetPredictedPlayer.ValueRW.LastHitTick = serverCurrentTick;

                        bool isFatal = (healthBeforeDamage > 0 && targetPredictedPlayer.ValueRO.CurrentHealth <= 0);
                        HitType hitType = isFatal ? HitType.Kill : (isHeadshot ? HitType.Headshot : HitType.Body);
                        HitmarkerManager.TriggerHit(hitType);
