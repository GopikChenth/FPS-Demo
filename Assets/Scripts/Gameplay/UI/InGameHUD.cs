...
namespace Unity.MP_FPS
{
    ...
    public class InGameHUD : MonoBehaviour
    {
        ...
        private List<VisualElement> m_HitmarkerTicks = new List<VisualElement>();
        private float m_HitmarkerTimer = 0f;
        private float m_HitmarkerDuration = 0.12f;
        private float m_HitmarkerStartScale = 1.0f;
        ...
        void OnEnable()
        {
            ...
            m_Hitmarker = m_RootElement.Q<VisualElement>("hitmarker");
            if (m_Hitmarker != null)
            {
                m_HitmarkerTicks = m_Hitmarker.Query<VisualElement>(className: "hitmarker-tick").ToList();
            }
        }
        ...
        void LateUpdate()
        {
            ...
            // Update Hitmarker Animation
            if (m_HitmarkerTimer > 0f)
            {
                m_HitmarkerTimer -= Time.deltaTime;
                float progress = Mathf.Clamp01(m_HitmarkerTimer / m_HitmarkerDuration);
                if (m_Hitmarker != null)
                {
                    m_Hitmarker.style.opacity = progress;
                    float scale = Mathf.Lerp(1.0f, m_HitmarkerStartScale, progress);
                    m_Hitmarker.style.scale = new StyleScale(new Scale(new Vector2(scale, scale)));
                }
            }
            else if (m_Hitmarker != null && m_Hitmarker.style.opacity.value > 0f)
            {
                m_Hitmarker.style.opacity = 0f;
            }
        }

        public void TriggerHitmarker(HitType hitType)
        {
            m_HitmarkerDuration = hitType == HitType.Kill ? 0.25f : (hitType == HitType.Headshot ? 0.18f : 0.12f);
            m_HitmarkerTimer = m_HitmarkerDuration;
            m_HitmarkerStartScale = hitType == HitType.Kill ? 1.4f : (hitType == HitType.Headshot ? 1.25f : 1.0f);
            Color hitColor = hitType == HitType.Kill
                ? new Color(1f, 0.2f, 0.2f)
                : (hitType == HitType.Headshot ? new Color(1f, 0.85f, 0.1f) : Color.white);

            if (m_Hitmarker != null)
            {
                m_Hitmarker.style.opacity = 1f;
                foreach (var tick in m_HitmarkerTicks)
                {
                    tick.style.backgroundColor = hitColor;
                }
            }
        }

        // Dynamic bloom fields
        private float m_BloomSpread = 0f;
        private float m_CurrentReticleSize = 80f;
        private uint _lastSeenShotTick = 0;
        ...
        private void UpdateReticleVisual(PredictedPlayerGhost playerData)
        {
            ...
            // 1. Stance & Movement Spread Calculation
            ...
            // 2. Shot Bloom Impulse
            if (playerData.LastShotTick > _lastSeenShotTick)
            {
                m_BloomSpread += 28f;
                _lastSeenShotTick = playerData.LastShotTick;
            }
            m_BloomSpread = Mathf.Lerp(m_BloomSpread, 0f, 10f * Time.deltaTime);

            float targetSize = reticleBaseSize * stanceMult * speedMult + m_BloomSpread;
            m_CurrentReticleSize = Mathf.Lerp(m_CurrentReticleSize, targetSize, 12f * Time.deltaTime);

            m_Reticle.style.width = m_CurrentReticleSize;
